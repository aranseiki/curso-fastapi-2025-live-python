...
async def read_users(
    ...
    users = await session.scalars(
        select(User).limit(filterPage.limit).offset(filterPage.offset)
    )
    return {'users': users}
...
async def update_user(
    ...
    try:
        ...
        raise HTTPException(
            status_code=HTTPStatus.CONFLICT,
            detail='Username or Email alrealdy exists',
        )
...
async def read_user_with_id(user_id: int, session: Session):  # type: ignore
